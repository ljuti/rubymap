    ...
    module Rails
      ...
      class ModelEnricher < BaseEnricher
        ...
        def is_activerecord_model?(klass)
          return false unless klass.parent_class
        ...
        def enrich_model(klass, result)
          ...
          model_info.association_count = model_info.associations.size
          model_info.validation_count = model_info.validations.size
          model_info.callback_count = model_info.callbacks.size
          model_info.scope_count = model_info.scopes.size
        ...
        def calculate_model_complexity(model_info)
          ...
          score += 5 if model_info.callbacks.any? { |c| c[:type].start_with?("around_") }
          score += 3 if model_info.associations.any? { |a| a[:type] == "has_many_through" }
          score += 2 if model_info.scopes.any? { |s| s[:type] == "default_scope" }
        ...
        def detect_model_issues(model_info)
          ...
          if model_info.scopes.any? { |s| s[:type] == "default_scope" }
            issues << {
