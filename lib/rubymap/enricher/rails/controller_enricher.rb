    ...
    module Rails
      ...
      class ControllerEnricher < BaseEnricher
        ...
        def is_rails_controller?(klass)
          return false unless klass.parent_class || klass.name&.end_with?("Controller")
          
          # Check if it inherits from a controller base class
          CONTROLLER_INDICATORS.any? do |indicator|
            klass.parent_class == indicator || 
            klass.ancestors&.include?(indicator)
        ...
        def enrich_controller(klass, result)
          controller_info = RailsControllerInfo.new(
            ...
            actions: extract_actions(klass),
            filters: extract_filters(klass),
            strong_parameters: extract_strong_parameters(klass),
            rescue_handlers: extract_rescue_handlers(klass),
          ...
          # Add metrics
          controller_info.action_count = controller_info.actions.size
          controller_info.filter_count = controller_info.filters.size
        ...
        def extract_response_format(method)
          ...
          method.calls_made&.each do |call|
            case call[:method]
            when "render"
              if call[:arguments]&.any? { |arg| arg.include?("json") }
                formats << "json"
        ...
        def calculate_rest_compliance(controller_info)
          rest_actions = controller_info.actions.select { |a| a[:type] == "rest" }
        ...
        def calculate_controller_complexity(controller_info)
          ...
          non_rest = controller_info.actions.count { |a| a[:type] != "rest" }
          score += non_rest * 2
        ...
        def detect_controller_issues(controller_info, klass)
          ...
          if controller_info.actions.any? { |a| a[:complexity] > 10 }
            issues << {
              type: "fat_controller",
              severity: "high",
          ...
          if controller_info.actions.any? { |a| %w[create update].include?(a[:name]) } &&
