    ...
    module Metrics
      ...
      class ApiSurfaceMetric < BaseMetric
        def calculate(result, config)
          api_threshold = config_value(config, :api_size_threshold, 20)
        ...
        def calculate_class_api_surface(klass, all_methods)
          ...
          # Alternative: use the method lists if visibility info is stored differently
          if klass.respond_to?(:visibility) && klass.visibility
            public_instance_count = count_public_methods(klass.instance_methods, klass.visibility)
            public_class_count = count_public_methods(klass.class_methods, klass.visibility)
          end
