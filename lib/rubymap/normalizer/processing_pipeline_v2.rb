...
module Rubymap
  class Normalizer
    ...
    # Simplified public interface - can be used as a drop-in replacement
    class ProcessingPipeline
      def initialize(container)
        @pipeline = ProcessingPipelineV2.new(container)
      end
      ...
      # Expose private methods for backward compatibility during migration
      # These can be removed once all tests are updated
      private

      def process_symbols(raw_data, result, errors)
        context = PipelineContext.new(input: raw_data, result: result, container: @pipeline.send(:container))
        context.errors = errors
        
        ExtractSymbolsStep.new.call(context)
        ProcessSymbolsStep.new.call(context)
      end

      def resolve_relationships(result)
        context = PipelineContext.new(input: nil, result: result, container: @pipeline.send(:container))
        ResolveRelationshipsStep.new.call(context)
      end

      def deduplicate_symbols(result)
        context = PipelineContext.new(input: nil, result: result, container: @pipeline.send(:container))
        DeduplicateSymbolsStep.new.call(context)
      end

      def format_output(result)
        context = PipelineContext.new(input: nil, result: result, container: @pipeline.send(:container))
        FormatOutputStep.new.call(context)
      end

      # Delegated methods for compatibility
      def extract_symbol_data(raw_data)
        ExtractSymbolsStep.new.send(:extract_symbol_data, raw_data)
      end

      def extractor_result?(raw_data)
        ExtractSymbolsStep.new.send(:extractor_result?, raw_data)
      end

      def convert_to_hashes(items)
        ExtractSymbolsStep.new.send(:convert_to_hashes, items)
      end

      def extract_from_hash(raw_data)
        ExtractSymbolsStep.new.send(:extract_from_hash, raw_data)
      end

      def extract_from_result(raw_data)
        ExtractSymbolsStep.new.send(:extract_from_result, raw_data)
      end

      def create_empty_symbol_data
        ExtractSymbolsStep.new.send(:empty_symbol_data)
      end

      def index_symbols(result)
        context = PipelineContext.new(input: nil, result: result, container: @pipeline.send(:container))
        ProcessSymbolsStep.new.send(:index_symbols, context)
      end
      ...
      def process_main_symbols(processor_factory, extracted_data, result, errors)
        ...
        context.extracted_data = extracted_data
        context.errors = errors
        ProcessSymbolsStep.new.send(:process_main_symbols, processor_factory, context)
      end

      def process_mixins(processor_factory, mixins, result, errors)
        context = PipelineContext.new(input: nil, result: result, container: @pipeline.send(:container))
        context.extracted_data = {mixins: mixins}
        context.errors = errors
        ProcessSymbolsStep.new.send(:process_mixins, processor_factory, context)
      end
    end
  end
end