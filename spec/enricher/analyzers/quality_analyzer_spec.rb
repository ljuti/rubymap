...
RSpec.describe Rubymap::Enricher::Analyzers::QualityAnalyzer do
  ...
  describe "#collect_quality_scores" do
    ...
    it "appends scores to array with <<" do
      method = double("method", quality_score: 0.8)
      klass = double("class", quality_score: 0.7)
      ...
      scores_array = []
      expect(scores_array).to receive(:<<).with(0.8).and_call_original
      expect(scores_array).to receive(:<<).with(0.7).and_call_original
      allow(analyzer).to receive(:collect_quality_scores).and_return(scores_array)
